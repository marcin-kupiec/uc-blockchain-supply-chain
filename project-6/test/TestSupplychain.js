...
contract('SupplyChain', function(accounts) {
    ...
    it("Testing smart contract function harvestItem() that allows a farmer to harvest coffee", async() => {
        ...
        assert.equal(resultBufferOne[0], sku, `Error: Invalid item SKU, expected ${sku}, received ${resultBufferOne[0]}`)
    ...
    it("Testing smart contract function fetchItemBufferTwo() that allows anyone to fetch item details from blockchain", async() => {
        ...
        assert.equal(result[0], sku, 'Error: Invalid item SKU')
        assert.equal(result[1], upc, 'Error: Invalid item UPC')
        assert.equal(result[2], productID, 'Error: Invalid productID')
        ...
        assert.equal(result[5], 7, 'Error: Invalid item State')
        assert.equal(result[6], distributorID, 'Error: Invalid distributorID')
        assert.equal(result[7], retailerID, 'Error: Invalid retailerID')
        assert.equal(result[8], consumerID, 'Error: Invalid consumerID')
    })
});
